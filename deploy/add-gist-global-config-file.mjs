/**
 * 在已有 Gist 里新增 global-config.json（只需运行一次）
 * node deploy/add-gist-global-config-file.mjs
 */
import fs from "fs";
...
const res = await fetch(`https://api.github.com/gists/${gistId}`, {
  ...
  body: JSON.stringify({
    files: { "global-config.json": { content: empty } },
  }),
});
...
console.log("✓ 已在 Gist 中创建 global-config.json");
